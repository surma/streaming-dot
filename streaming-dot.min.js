(function(a,b){'object'==typeof exports&&'string'!=typeof exports.nodeName?b(exports):b(a.doT={})})(this,function(a){'use strict';function b(d){return d.replace(/\\('|\\)/g,'$1').replace(/[\r\t\n]/g,' ')}Object.assign(a,{version:'1.0.0',templateSettings:{evaluate:/\{\{(([^\}]+|\\.)+)\}\}/g,interpolate:/\{\{=\s*([^\}]+)\}\}/g,stream:/\{\{~\s*([^\}]+)\}\}/g,conditional:/\{\{\?\s*([^\}]*)\}\}/g,node:'object'==typeof process,varname:'it'}}),a.compile=function(d,g){g=Object.assign({},a.templateSettings,g);var h;h=g.node?`var s=(r)=>{
var d=!1,l,b=[];
r.then(r=>{
r.on('end',_=>{d=!0;l&&l()});
...
};`,d='var P=Promise.resolve.bind(Promise);'+h+'var g=function*(){yield P(\''+d.replace(/'|\\/g,'\\$&').replace(g.interpolate,function(i,j){return'\');yield P('+b(j)+');yield P(\''}).replace(g.conditional,function(i,j){return j?'\');yield P('+b(j)+').then(v=>v&&P(\'':'\'));yield P(\''}).replace(g.stream,function(i,j){return'\');yield* s(P('+b(j)+'));yield P(\''}).replace(g.evaluate,function(i,j){return'\');'+b(j)+';yield P(\''}).replace(/\n/g,'\\n').replace(/\t/g,'\\t').replace(/\r/g,'\\r')+'\');}();',d+=g.node?`var r = new R({read:function f() {
var d=g.next();
