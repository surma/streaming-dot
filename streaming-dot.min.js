(function(a,b){'object'==typeof exports&&'string'!=typeof exports.nodeName?b(exports):b(a.doT={})})(this,function(a){'use strict';function b(d){return d.replace(/\\('|\\)/g,'$1').replace(/[\r\t\n]/g,' ')}Object.assign(a,{version:'1.1.0',templateSettings:{evaluate:/\{\{(([^\}]+|\\.)+)\}\}/g,interpolate:/\{\{=\s*([^\}]+)\}\}/g,stream:/\{\{~\s*([^\}]+)\}\}/g,conditional:/\{\{\?(\?)?\s*([^\}]*)?\}\}/g,node:'object'==typeof process,varname:'it'}}),a.compile=function(d,g){g=Object.assign({},a.templateSettings,g);var h;h=g.node?`var s=(r)=>{
var d=!1,l,b=[];
r.then(r=>{
r.on('end',_=>{d=!0;l&&l()});
...
};`,d='var P=Promise.resolve.bind(Promise);function* f(p,a,b=_=>[]){yield p.then(v=>(a=v?a:b)&&\'\');yield* a();}'+h+'var g=function*(){yield P(\''+d.replace(/'|\\/g,'\\$&').replace(g.interpolate,function(i,j){return'\');yield P('+b(j)+');yield P(\''}).replace(g.conditional,function(i,j,k){if(k&&!j)// {{?<something>}} === if
return'\');yield* f(P('+b(k)+'),function*(){yield P(\'';return!k&&j?'\')},function*(){yield P(\'':'\')});yield P(\'';// {{?}} === "endif"
