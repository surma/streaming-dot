...
};`,d='var P=Promise.resolve.bind(Promise);function* f(p,a,b){yield p.then(v=>(a=v?a:b)&&\'\');yield* (a||(_=>[]))();}'+h+'var g=function*(){yield P(\''+d.replace(/'|\\/g,'\\$&').replace(g.interpolate,function(i,j){return'\');yield P('+b(j)+');yield P(\''}).replace(g.conditional,function(i,j,k){if(k&&!j)// {{?<something>}} === if
